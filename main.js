...
// src/folder-render.ts
var FolderRender = class {
  constructor(app) {
    this.app = app;
  }
  async run(source, el, ctx) {
    var _a;
    let yaml;
    try {
      yaml = parse(source);
    ...
    var mappedAndSortedFiles = subFileList.map((subFilePath2) => {
      var _a2;
      let abstractFile = this.app.vault.getAbstractFileByPath(subFilePath2);
      if (abstractFile instanceof import_obsidian.TFile) {
        let metadata = (_a2 = this.app.metadataCache.getFileCache(abstractFile)) == null ? void 0 : _a2.frontmatter;
        let date = this.parseBirthDate(metadata == null ? void 0 : metadata.birth_date);
        let hasDesiredTag = tags.every((tag) => ((metadata == null ? void 0 : metadata.tags) || []).includes(tag));
        return { subFilePath: subFilePath2, date, valid: hasDesiredTag };
      }
      return { subFilePath: subFilePath2, date: 0, valid: false };
    }).filter((file) => file.valid).sort((a, b) => a.date - b.date);
  ...
  createCard(folder, metadata, basename) {
    ...
    if (this.app.workspace.activeLeaf) {
      import_obsidian.MarkdownRenderer.renderMarkdown(`### [[${folder}/${basename}.md|${basename}]]`, cardContentEl, "", this.app.workspace.activeLeaf.view);
    }
    let titleMD = "";
...
*${metadata.subtitle}*`;
    ...
    if (this.app.workspace.activeLeaf) {
      import_obsidian.MarkdownRenderer.renderMarkdown(titleMD, cardContentEl, "", this.app.workspace.activeLeaf.view);
    }
    if (metadata && metadata.authors) {
      ...
      if (this.app.workspace.activeLeaf) {
        import_obsidian.MarkdownRenderer.renderMarkdown(authorLinks, cardContentEl, "", this.app.workspace.activeLeaf.view);
      }
    }
    let figureMD = "";
...
**Death place:** ${metadata.death_place}`;
    ...
    if (this.app.workspace.activeLeaf) {
      import_obsidian.MarkdownRenderer.renderMarkdown(figureMD, cardContentEl, "", this.app.workspace.activeLeaf.view);
    }
    let statusMD = "";
...
**Updated:** ${metadata.status_updated}`;
    ...
    if (this.app.workspace.activeLeaf) {
      import_obsidian.MarkdownRenderer.renderMarkdown(statusMD, cardContentEl, "", this.app.workspace.activeLeaf.view);
    }
    if (metadata && metadata.tags) {
      ...
      if (this.app.workspace.activeLeaf) {
        import_obsidian.MarkdownRenderer.renderMarkdown(tagsText, cardContentEl, "", this.app.workspace.activeLeaf.view);
      }
    }
    if (metadata && metadata.cover) {
      ...
      if (coverImageFile instanceof import_obsidian.TFile) {
...
function escapeYAMLForbiddenChars(input) {
  if (input == void 0) {
    return "";
  }
...
function getFileUniqueName(app, fileName) {
  let attempt = 0;
  let exist;
  let newFileName = "";
  while (true) {
...
async function downloadImage(url, path) {
  ...
  const image = await (0, import_obsidian2.getBlobArrayBuffer)(imageblob);
  try {
    ...
    try {
      ...
      const books = data.items.map((item) => {
        ...
        return {
          ...
          coverURL: (_a = item.volumeInfo.imageLinks) == null ? void 0 : _a.smallThumbnail,
          publisher: item.volumeInfo.publisher,
          published: item.volumeInfo.publishedDate,
          isbn: ((_b = item.volumeInfo.industryIdentifiers) == null ? void 0 : _b.map((identifier) => `${identifier.identifier}`)) || [],
...
var CreateBook = class extends import_obsidian4.Modal {
  ...
  processCategories(categories1, categories2) {
    if (categories1 == void 0 && categories2 == void 0) {
      return [];
    }
    let categories = [];
    if (categories1 == void 0) {
      categories = categories2;
    } else if (categories2 == void 0) {
      categories = categories1;
    } else {
      categories = [...categories1, ...categories2];
    }
    const allItems = categories.flatMap(
      (item) => item.split(/[&/]/).map((subItem) => subItem.trim().split(/\s+/).join("-"))
    );
  ...
  getOldestDate(dateString1, dateString2) {
    if (dateString1 == void 0 && dateString2 == void 0) {
      return "";
    }
    if (dateString1 == void 0) {
      return dateString2;
    }
    if (dateString2 == void 0) {
      return dateString1;
    }
  ...
  async fetchBookFurtherDetails() {
    ...
    try {
      ...
      this.book.coverURL = ((_a = bookByID.volumeInfo.imageLinks) == null ? void 0 : _a.smallThumbnail) || this.book.coverURL;
  ...
  async onload() {
    ...
    this.app.workspace.on("receive-text-menu", (menu, shareText) => {
      menu.addItem((item) => {
        ...
        item.setIcon("book");
        item.onClick(() => {
          new SearchBook(this.app, shareText).open();
        });
      });
    }), // add figure command
    this.addCommand({
    ...
    this.app.workspace.onLayoutReady(async () => {
      this.registerEvent(this.app.vault.on(
        "create",
        (file) => onFigureCreation(this.app, file)
      ));
    });
    this.addCommand({
      id: "add-article",
    ...
    this.app.workspace.onLayoutReady(async () => {
      this.registerEvent(this.app.vault.on(
        "create",
        (file) => onPostCreation(this.app, file)
      ));
    });
    this.addCommand({
      id: "clean-covers",
